...
function save_basic_anim_panel(fig_path, ell, q1, q2, ψ1, ψ2, U_bg, t)
    plotname = "snapshots"
    ...
    pc4 = ax[4].contourf(x, y, (ψ2 .- mean(ψ2, dims=1))', cmap=PyPlot.cm.PiYG, vmin=-ax4lim, vmax=ax4lim, levels=7)
    ax[4].set_title("ψ2")

    plt.colorbar(pc4)

    ax1lim = maximum(abs.(q1 .- mean(q1, dims=1)))
    pc = ax[1].pcolormesh(x, y, (q1 .- mean(q1, dims=1))', cmap=PyPlot.cm.bwr, vmin=-ax1lim, vmax=ax1lim)
    ax[1].set_title("q1")

    plt.colorbar(pc)

    ax3lim = maximum(abs.(q2 .- mean(q2, dims=1)))
    pc2 = ax[2].pcolormesh(x, y, (q2 .- mean(q2, dims=1))', cmap=PyPlot.cm.bwr, vmin=-ax3lim, vmax=ax3lim)
    ax[2].set_title("q2")
...
function save_BT_anim_panel(fig_path, ell, q, ψ)
    ...
    pc = ax.pcolormesh(x ./ Ld, y ./ Ld, (q .- f0)', cmap=PyPlot.cm.Oranges) #, vmin=-lim, vmax=lim)
