...
U0 = 0.25                         # Internal shear forcing (0.5 m/s oceanic jet)
trans = 1.0       		  # trans=0 means flat baroclinic zone, trans=1 means PH96 baroclinic zone
WC = 200e3                        # half-width baroclinic zone
...
r = 1.0 / (30.0 * 24 * 3600)      # Oceanic bottom friction 
α = 0 # 1.0 / (10.0 * 24 * 3600)      # Thermal relaxation
...
h0_topo = 2000.0       # Maximum height of the continental shelf (m)
W_shelf = 100e3        # Shelfbreak width 
y_shelf = 0.0         # Central latitude of the shelf break

# Tanh profile: shallow on the Antarctic continent (south), deep offshore (north)
...
################################################################################
τ0 = 0.15          # Maximum wind stress (N/m^2)
W_wind = 400e3   # Total width of the central wind forcing zone 
...
r_taper_start = 0                   # Where the taper is centered
...
################################################################################
base_name = "fixUbool$(fix_zonal_mean)_beta$(beta)_WC$(WC)_trans$(trans)_r$(r)_h0topo$(h0_topo)_Wshelf$(W_shelf)_U0$(U0)_tau0$(τ0)_alpha$(α)"
