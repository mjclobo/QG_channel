...
WC = 30  # Width of boundary where background flow decays to zero (max of 0.5)
...
# this saves meridional chunks (full zonal extent) of i) ψ1, ii) ψ2, and iii) t
save_bool = true
save_path = "/home/matt/Desktop/research/QG/channel_model/data/Lee_1997_WC20/"
...
plot_bool = true
fig_path = "/home/matt/Desktop/research/QG/channel_model/anim/Lee_1997_WC20/"
...
################################################################################
ν = 6e-3 #  0.01 * dx^4 / dt # 1e6          # Hyperviscosity (m⁴/s)
...
# # alternately initialize with the end of a previous run
restart_path = "/home/matt/Desktop/research/QG/channel_model/data/Lee_1997_init/"
...
for (i, file) in enumerate(ψfiles)
    t_array[i] = extract_time(restart_path*file)
end
...
################################################################################
cmax = 10.0          # maximum phase speed considered
ncbins = Nx             # number of phase speed bins; should be some factor of Nx
...
block_len = round(Int, length(t_array)/5)
block_stride = round(Int, 0.75 * block_len)
