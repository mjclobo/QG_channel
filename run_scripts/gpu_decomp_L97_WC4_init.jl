...
include("/home/matt/Desktop/research/QG/cpu_gpu_zonal_mean_decomp_2LQG.jl")

################################################################################
# Define background flow profile
################################################################################

WC = 13  # Width of boundary where background flow decays to zero (max of 0.5)
...
plot_every = round(Int,nt/40)      # period of plot output frequency
...
q1_bg = zeros(size(ψ1_bg))
q2_bg = zeros(size(ψ2_bg))

compute_qg_pv_bar!(q1_bg, q2_bg, ψ1_bg, ψ2_bg, laplacian1D_free_slip_y!, dy)
...
t0 = 0.0    # initial timestamp (in seconds)

################################################################################
# Run model from initial conditions
################################################################################
include("../define_vars.jl")
params = ModelParams(Nx, Ny, nt, Lx, Ly, dt, beta, f0, g, [H1, H2], ρ0, Δρ, ν, r, α, U0, WC, ψ_diff_bg)

rhs_ws = make_rhs_workspace(Nx, Ny)    # preallocated arrays
rk4_ws = RK4Workspace(Nx, Ny, rhs_ws)

run_model_decomp_alloc!(q1_bar, q2_bar, q1_prime, q2_prime, t0, params;
                        nt=nt, dt=dt, rk4_ws=rk4_ws, output_every=500)
