...
plot_BCI_bool = false
fig_path = "/home/matt/Desktop/research/QG/QG_channel_output/anim/L97_WC4_SS_momentum/"
plot_every = round(Int,nt/20)      # period of plot output frequency

plotname="snapshot_mom_budget"
################################################################################
...
for (i, t) in enumerate(t_array[1:4:end])
    ...
    # define thermal damping term
    therm1_bar =  α * mean((u1_snap .- u2_snap) .- U_bg[save_ind_start:save_ind_end]', dims=1)
    therm2_bar = -α * mean((u1_snap .- u2_snap) .- U_bg[save_ind_start:save_ind_end]', dims=1)
    ...
    ax[10].plot(therm1_bar', y[save_ind_start:save_ind_end], alpha=0.5, label=L"\mathrm{therm. forc.}")

    ax[10].plot((v1ζ1_bar .- (f0 * v1η_bar ./ H1) .+ f0 .* v1_bar_star .+ therm1_bar)', y[save_ind_start:save_ind_end], "k-", linewidth=2, label=L"\partial_{t} \, \overline{u}_{1}")
    ...
    ax[12].plot(therm2_bar', y[save_ind_start:save_ind_end], alpha=0.5, label=L"\mathrm{therm. forc.}")

    ax[12].plot((v2ζ2_bar .- (f0 * v2η_bar ./ H2) .+ f0 .* v2_bar_star .+ therm2_bar .+ drag)', y[save_ind_start:save_ind_end], "k-", linewidth=2, label=L"\partial_{t} \, \overline{u}_{2}")
    ...
    therm1_bar_tm .+= therm1_bar'

    v2_bar_star_tm .+= v2_bar_star'
    v2η_bar_tm .+= v2η_bar'
    v2ζ2_bar_tm .+= v2ζ2_bar'
    drag_tm .+= drag'
    therm2_bar_tm .+= therm2_bar'
...
ax[1].plot((therm1_bar_tm ./ mean_count)', y[save_ind_start:save_ind_end], alpha=0.5, label=L"\mathrm{therm. forc.}")
...
ax[2].plot((therm2_bar_tm ./ mean_count)', y[save_ind_start:save_ind_end], alpha=0.5, label=L"\mathrm{therm. forc.}")
...
plt.plot(((v1ζ1_bar_tm .- (f0 * v1η_bar_tm ./ H1) .+ f0 .* v1_bar_star_tm) ./ mean_count)', y[save_ind_start:save_ind_end], "k-", linewidth=2, label=L"\partial_{t} \, \overline{u}_{1}")
